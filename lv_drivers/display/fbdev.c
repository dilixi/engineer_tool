...
static int fbfd = 0;
extern uint16_t *p_fb_buf_active;
...
void fbdev_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    if(fbp == NULL ||
            area->x2 < 0 ||
            area->y2 < 0 ||
            area->x1 > (int32_t)drv->hor_res - 1 ||
            area->y1 > (int32_t)drv->ver_res - 1) {
    ...
    int32_t act_x2 = area->x2 > (int32_t)drv->hor_res - 1 ? (int32_t)drv->hor_res - 1 : area->x2;
    int32_t act_y2 = area->y2 > (int32_t)drv->ver_res - 1 ? (int32_t)drv->ver_res - 1 : area->y2;
    ...
    //if(drv->bits_per_pixel == 16) 
    //if (drv->draw_buf->flushing_last)
    { 
        //printf("bits_per_pixel 16\r\n");
        uint16_t * fbp16 = (uint16_t *)p_fb_buf_active;
        int32_t y;
        for(y = act_y1; y <= act_y2; y++) {
            location = (act_x1 + 0) + (y + 0) * drv->hor_res*2 / 2;
            memcpy(&fbp16[location], (uint32_t *)color_p, (act_x2 - act_x1 + 1) * 2);
            color_p += w;
        }   
        
        //memcpy((uint8_t *)p_fb_buf_active, color_p, drv->hor_res*drv->ver_res*2);
    }
    #endif
