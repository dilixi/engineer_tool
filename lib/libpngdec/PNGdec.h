...
typedef struct
{ 
    uint16_t width; // size of this line
    uint16_t height; // bytes per line 
    uint32_t bg_color;
    uint8_t *pPixels;
    uint16_t *u16_buffer;
    void *p_png_tool;
...
typedef struct png_image_tag
{
    ...
    PNGFILE PNGFile;
    uint8_t ucZLIB[32768 + sizeof(inflate_state_t)]; // put this here to avoid needing malloc/free
