...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
...
#if LV_USE_MEM_MONITOR && LV_USE_LABEL
    ...
    if(lv_tick_elaps(mem_monitor.mem_last_time) > 300) {
        ...
        uint32_t total_size = mon.total_size;
        uint32_t total_kb = total_size / 1024;
        uint32_t total_kb_tenth = (total_size - (total_kb * 1024)) / 102;
        uint32_t left_size = mon.free_size;
        uint32_t left_kb = left_size / 1024;
        uint32_t left_kb_tenth = (left_size - (left_kb * 1024)) / 102;
        #if LV_MEM_CUSTOM
        lv_label_set_text_fmt(mem_label,
                              "%"LV_PRIu32 ".%"LV_PRIu32 " kB used (%d %%)\n"
                              "%"LV_PRIu32 ".%"LV_PRIu32 " kB total\n"
                              "%"LV_PRIu32 ".%"LV_PRIu32 " kB left",
                              used_kb, used_kb_tenth, mon.used_pct,
                              total_kb, total_kb_tenth,
                              left_kb, left_kb_tenth);
        #else
        lv_label_set_text_fmt(mem_label,
                              "%"LV_PRIu32 ".%"LV_PRIu32 " kB used (%d %%)\n"
                              "%d%% frag.",
